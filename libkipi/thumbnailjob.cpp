...
bool ThumbnailJob::statThumbnail()
{
    ...
    if (d->dir) {
        QString dirFile(thumb.text("KIPI::Highlight",0));
        int  dirFileMTime = thumb.text("KIPI::MTime",0).toInt();
...
void ThumbnailJob::slotThumbData(KIO::Job*, const QByteArray &data)
{
    ...
    if (!d->dir) {
        ...
        thumb.setText(QString("Software").latin1(),
                      0, QString("KIPI Thumbnail Generator"));
    }
    else {
        thumb.setText(QString("KIPI::Highlight").latin1(),
                      0, d->curr_dirfile);
        thumb.setText(QString("KIPI::MTime").latin1(),
